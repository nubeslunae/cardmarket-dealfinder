#!/usr/bin/env node
// Conditie-prijzen (NM/LP/MP/HP/DMG, TCGplayer via JustTCG, USD) voor de sets met de meeste top-deals.
// Zuinig: gratis tier = 1.000 calls/maand, 100/dag, 10/min, 20 kaarten per set-pagina. Standaard max 25 calls
// per run, rondlopend over set-pagina's. Vorige resultaten komen van de live site; ouder dan 30 dagen vervalt.
//
...
async function main() {
  ...
  const found = new Map(); const calls1 = 0;
  // Set-pagina's: sets gerangschikt op aantal top-deals (avg7 - low), rondlopende offset per set
